...
/// Array table map.
///
/// High level API for BPF_MAP_TYPE_ARRAY maps.
#[repr(transparent)]
...
impl<K, V> ArrayMap<K, V> {
    ...
    pub const fn with_max_entries(max_entries: u32) -> Self {
        Self {
            def: bpf_map_def {
                type_: bpf_map_type_BPF_MAP_TYPE_ARRAY,
                key_size: mem::size_of::<K>() as u32,
                value_size: mem::size_of::<V>() as u32,
                max_entries,
                map_flags: 0
...
/// Per CPU Array table map.
///
/// High level API for BPF_MAP_TYPE_PERCPU_ARRAY maps.
#[repr(transparent)]
pub struct PerCPUArrayMap<K, V> {
    def: bpf_map_def,
    _k: PhantomData<K>,
    _v: PhantomData<V>,
}

impl<K, V> PerCPUArrayMap<K, V> {
    /// Creates a map with the specified maximum number of elements.
    pub const fn with_max_entries(max_entries: u32) -> Self {
        Self {
            def: bpf_map_def {
                type_: bpf_map_type_BPF_MAP_TYPE_PERCPU_ARRAY,
                key_size: mem::size_of::<K>() as u32,
                value_size: mem::size_of::<V>() as u32,
                max_entries,
                map_flags: 0
